...
{
    public sealed class BagEquipEvent : EquipEvent
    {
        [SerializeField] private StatValue _armorValue = new(StatType.Armor, 5f, StatValueType.Flat);
        [SerializeField] private StatValue _resistanceValue = new(StatType.Resistance, 5f, StatValueType.Flat);

        public string Reason => "Bag equipment";
        ...
        private void Creature_AfterTakeDamage(object sender, DamageContainer container)
        {
            ...
            if (creature.Equipment is not IHasBackpackSlot backpackSlot) return;

            backpackSlot.BackpackSlot.DeductDurability(container.Damage * DeductDurabilityScale);
        }
